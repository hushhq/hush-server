...
# Hosted-deploy guard
# ---------------------------------------------------------------------------
# update.sh is for self-hosted instances. If a host-specific compose override
# (docker-compose.app.*.yml) is present, this host uses a different compose
# setup and update.sh will not do the right thing.
for _override in docker-compose.app.*.yml; do
...
if compose_cmd ps postgres 2>/dev/null | grep -qE "Up|running"; then
  ...
  # Metadata alongside the SQL dump
  _meta_file="${BACKUP_FILE%.sql}.meta"
  _timestamp="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  _backup_basename="$(basename "$BACKUP_FILE")"
  _git_sha="$(git -C "$PROJECT_ROOT" rev-parse HEAD 2>/dev/null || echo "unknown")"
  _git_version="$(git -C "$PROJECT_ROOT" describe --tags --exact-match HEAD 2>/dev/null || echo "untagged")"
  _livekit_version="$(grep -m1 'livekit/livekit-server:' "$PROJECT_ROOT/docker-compose.prod.yml" 2>/dev/null \
    | sed 's/.*livekit-server://' | tr -d ' \t')"

  cat > "$_meta_file" <<METAEOF
# Hush backup metadata — read-only, generated by update.sh
# A restored database is INOPERABLE without the .env that matches this backup.
timestamp=${_timestamp}
backup_file=${_backup_basename}
hush_server_git_sha=${_git_sha}
hush_server_version=${_git_version}
livekit_version=${_livekit_version:-unknown}
